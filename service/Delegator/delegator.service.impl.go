...
func (s *DelegatorServiceImpl) Create(ownerID string, req request.CreateDelegatorRequest) *helper.ErrorModel {
	...
	overlapping, errCheck := s.DelegatorRepository.HasOverlappingDelegation(ownerID, startDate, endDate, nil)
	if errCheck != nil {
		return errCheck
	}
	if overlapping {
	...
	return s.DelegatorRepository.Create(delegator)
}

func (s *DelegatorServiceImpl) GetAll(ownerID string) ([]response.DelegatorResponse, *helper.ErrorModel) {
	delegators, err := s.DelegatorRepository.GetAllByOwnerID(ownerID)
	if err != nil {
		return nil, err
	}
...
func (s *DelegatorServiceImpl) Update(id string, ownerID string, req request.UpdateDelegatorRequest) *helper.ErrorModel {
	if errs := helper.ValidateStruct(req); len(errs) > 0 {
		msg := "Invalid request data"
		return helper.ErrorCatcher(fmt.Errorf("validation failed"), 400, &msg)
	}

	existing, errGet := s.DelegatorRepository.GetByID(id)
	if errGet != nil {
		return errGet
	}
	...
	idStr := id
	overlapping, errCheck := s.DelegatorRepository.HasOverlappingDelegation(ownerID, startDate, endDate, &idStr)
	if errCheck != nil {
		return errCheck
	}
	if overlapping {
	...
	return s.DelegatorRepository.Update(*existing)
}

func (s *DelegatorServiceImpl) Delete(id string, ownerID string) *helper.ErrorModel {
	existing, errGet := s.DelegatorRepository.GetByID(id)
	if errGet != nil {
		return errGet
	}
	...
	return s.DelegatorRepository.Delete(id)
}
...
func (s *DelegatorServiceImpl) ResolveDelegate(ownerID string, date time.Time) (string, *helper.ErrorModel) {
	current := ownerID
	for depth := 0; depth < maxDelegationDepth; depth++ {
		delegation, err := s.DelegatorRepository.GetActiveDelegationByOwnerID(current, date)
		if err != nil {
			return "", err
		}
		if delegation == nil {
...
func (s *DelegatorServiceImpl) GetOwnerChainForDelegate(delegateID string, date time.Time) ([]string, *helper.ErrorModel) {
	...
	for len(queue) > 0 && len(result) < maxDelegationDepth*10 {
		...
		ownerIDs, err := s.DelegatorRepository.GetOwnerIDsByDelegateID(current, date)
		if err != nil {
			return nil, err
		}
